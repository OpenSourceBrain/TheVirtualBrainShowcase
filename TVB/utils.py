...
def get_2_region_conn(weight_between=0):
    ...
    conn.tract_lengths = numpy.array([[0,0],[0,0]])
...
def get_1_region_conn():
    ...
    conn.tract_lengths = numpy.array([[0]])
...
def model_info(model):
    print('++++++++++++++++= Model info +++++++++++++++++++')
    #print(dir(model))
    print('+ State vars:       %s'% str(model.state_variables))
    print('+ Vars of interest: %s'% str(model.variables_of_interest))
    '''
    print('+ Traits:           ')
...
def run_model(model, connectivity, duration, dt=0.01, coupling_linear=numpy.array([0.0]), initial_conditions=None):
